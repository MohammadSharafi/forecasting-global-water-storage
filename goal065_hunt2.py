"""Does a spatially smoothed anchor add to the AR correction? Leave-one-layout-out, validation only.

The AR result showed something specific: a quantity the model already holds as a FEATURE can still
pay when applied as a post-hoc linear term, because a boosted tree cannot cheaply represent
`model + a*x - b*y` through splits. The great-circle smoothed anchors (sa300 ... sa2500) are exactly
such quantities -- they are features of the shipped model, and they are a different kind of
information from the AR term: AR rescales the cell's OWN last observation per cell and horizon,
while sa is the neighbourhood's last observation. Complementary, in principle.

Same contract as ar_blend.py: weights sum to one, fitted on four layouts, scored on the fifth,
never on a leaderboard number. Results go to out/goal065/hunt2.txt, not to heldout.txt, whose last
line must stay the ADOPTED-SET of the closed goal.
"""
import glob
...
def main():
    ...
    for L in LAYOUTS:
        ...
        # layout_data drops non-finite rows, so rebuild the same mask to align the anchor columns
