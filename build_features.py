"""Memory-lean: build the v4 training matrix and the test matrix ONCE, write float32
parquet, free everything. Run alone (peak ~4-5 GB)."""
...
Xte.select(["ID","lat","lon","time","t_known","horizon","tws_known"]+F).with_columns([pl.col(c).cast(pl.Float32) for c in F]).write_parquet("out/Xte_v4.parquet")
print(f"test matrix {Xte.shape} written ({time.time()-t0:.0f}s)",flush=True); del Xte, te, obs_all, known, rows_te; gc.collect()
# --- training matrix (one row draw) ---
...
Xtr.select(["target","tws_known"]+F).with_columns([pl.col(c).cast(pl.Float32) for c in ["target","tws_known"]+F]).write_parquet(f"out/Xtr_v4_s{SEED}.parquet")
