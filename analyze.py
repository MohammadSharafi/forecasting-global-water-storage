"""Where the remaining error actually is (session 9n).

Every number this project has ever quoted is a single RMSE. That tells you whether a change
helped; it never tells you WHERE the error lives, and so every new idea has been proposed from
intuition rather than from the residuals. This is the anatomy: one pass over a layout's
validation predictions, decomposed every way that could suggest a next move.

Sections
--------
  horizon        RMSE and bias per horizon, plus each horizon's SHARE of the test-weighted MSE --
                 the only ranking that says where effort is worth spending
  vs persistence how much the model beats the last observation at each horizon, and the fraction
                 of rows where it is actually worse. A model that loses to persistence on a third
                 of rows has a routing problem, not an accuracy problem
  latitude       RMSE and bias per 30-degree band, and each band's share of total MSE
  regime         RMSE by the cell's own TWS variability (deciles of csd, from history). Error
                 concentrates in high-variance cells; if it concentrates MORE than variance does,
                 the model is failing where it matters
  season         RMSE by calendar month of the target
  staleness      RMSE by how many months the last observation is old -- the same thing as horizon
                 in this design, kept separate because a block's first month is special
  spatial        the residual variogram: correlation of the residual with its neighbour at grid
                 lags 1..5. If residuals are still correlated at lag 2-3, more smoothing is
                 available; if the correlation is gone by lag 1, smoothing is finished
  hotspots       the cells and the (band, horizon) cells contributing most of the total MSE

usage: python analyze.py A [stem[:tag],stem[:tag]...]
"""
import sys
import numpy as np
import polars as pl
...
def table(name, keys, y, p, k, order=None):
    """RMSE / bias / MSE share by a grouping key."""
...
def main():
    ...
    # ---- horizon, with the share of the TEST-weighted MSE rather than this layout's
    ...
    # ---- how often the model is simply worse than doing nothing
    print("\n  VS PERSISTENCE")
    for i in range(1, 8):
        m = h == i
        if m.sum() < 50:
    ...
    # how variable each cell is over this layout's own target months. Not a feature, not used
    # by anything -- it only says whether the error concentrates where the variance is.
    ...
    # ---- residual variogram: is there structure left for smoothing to take?
