"""Per-cell local-deviation persistence.  For each cell, history tells us how much of the cell's
departure from its 300/500 km neighbourhood mean carries into next month:
    (target - sa) ~ alpha_i + beta_i * (TWS_t - sa)
Fitted per cell with leave-one-year-out sums, so a training row never sees its own year.
Coordinates are used only to group a cell's own history; they are not model features.
This is information no model can recover here, because cell identity is not an input."""
...
# fit sample: history rows with an observed target (h=1 relationship), sa from history-only observations
...
for part,obs in (("tr",obs_hist),("va",obs_eval)):
    ...
    for km in RAD:
        ...
        cols[f"lp{km}"]=(sa+aa+bb*dev).astype(np.float32)              # local-persistence predictor
        cols[f"lpd{km}"]=(sa+aa+bb*dev-mk).astype(np.float32)          # its correction to the raw anchor
