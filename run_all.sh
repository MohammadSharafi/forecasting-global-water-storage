#!/bin/sh
# One command for the overnight run (session 9d).
#
#   ./run_all.sh          # ~6-8 h, unattended
#
# 1. horizon breakdown of the session-9c ablation, before anything is rebuilt
# 2. pipeline13: the 7-experiment grid on both layouts, scored under the test horizon mix
# 3. select_config.py: pick the winning configuration (must win on BOTH layouts)
# 4. pipeline14: rebuild FINAL with that configuration and write the submissions
#
# Nothing already on disk is destroyed: out/backup_pre_anom keeps the 0.709259 file, and
# out/sub_n_anom_lgbxgb.csv survives untouched, so there is always something to upload.
...
# --------------------------------------------------------------- 1. 9c horizon breakdown
# Must run BEFORE pipeline13 rebuilds the matrices, while these predictions are current.
