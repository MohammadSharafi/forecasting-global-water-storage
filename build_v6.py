"""Build v6 test predictions (3 seeds), write sub_v6 and the v5/v6 blend (+smoothed)."""
...
preds=[]
for s in (0,1,2):
    m=lgb.train({**P,"seed":s},ds,num_boost_round=270); m.save_model(f"out/lgb_final6_s{s}.txt"); preds.append(m.predict(Xt)+kt); print(f"  seed {s} ({time.time()-t0:.0f}s)",flush=True)
p6=np.mean(preds,axis=0); ids=Xte["ID"]
pl.DataFrame({"ID":ids,"Target":np.round(p6,6)}).write_csv("out/sub_v6_6dp.csv",float_precision=6)
v5=pl.read_csv("out/sub_v5_6dp.csv").join(pl.DataFrame({"ID":ids}).with_row_index("i"),on="ID").sort("i")["Target"].to_numpy()
pb=0.5*p6+0.5*v5
pl.DataFrame({"ID":ids,"Target":np.round(pb,6)}).write_csv("out/sub_v56_blend_6dp.csv",float_precision=6)
ps=smooth(Xte.select(["lat","lon","time","tws_known"]),pb,w=0.7,radius=1,iters=1)
pl.DataFrame({"ID":ids,"Target":np.round(ps,6)}).write_csv("out/sub_v56_blend_smooth_6dp.csv",float_precision=6)
print(f"wrote sub_v6, sub_v56_blend, sub_v56_blend_smooth  rows={len(p6)}  RMSE(v6 vs v5)={np.sqrt(np.mean((p6-v5)**2)):.4f}")
