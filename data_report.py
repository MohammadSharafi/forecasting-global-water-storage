"""What is actually in Train.csv and Test.csv (session 9n).

Nothing in this project has ever described the data itself. Every design decision -- the block
layouts, the horizon mix, HMIX=test, layout C -- rests on beliefs about the test file's structure
that were inferred once, by hand, and never re-derived from the file. This script derives them and
says whether they are right, then characterises the data the model has to work with.

What it reports
---------------
  coverage      rows, cells, months, and the GRACE gaps -- which calendar months are simply absent
  test geometry the block structure READ OFF Test.csv: run lengths, which months carry an
                observed TWS, the real gap between blocks, and the resulting horizon mix. This is
                cross-checked against eval_mix.TEST_BLOCKS, which everything else depends on: if
                the file disagrees, the horizon weighting, HMIX=test and layout C are all wrong.
  regime        per-era TWS statistics. Train is 2002-2015, the test starts in the GRACE/GRACE-FO
                gap and runs to 2018. If the distribution moved, that is the single fact that
                explains why validation gains transfer badly, and it is worth knowing its size.
  seasonality   amplitude of the mean annual cycle per latitude band, and how much of a cell's
                variance is seasonal -- an upper bound on what climatology alone can do
  persistence   lag-1 autocorrelation and the mean absolute one-month change, by band, which is
                what any h=1 model has to beat
  covariates    null rates, so a feature that is 40% missing is not mistaken for a weak one

usage: python data_report.py
"""
import sys
...
def runs(ms):
    """Consecutive-month runs in a sorted list of dates -> [(first, length), ...]."""
    out = []
    for m in ms:
...
def main():
    ...
    if mask_col:
        # the horizon each row will actually be predicted at: months since the last observed month
