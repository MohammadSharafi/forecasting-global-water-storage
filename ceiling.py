"""How much of the TWS change do the covariates actually explain? (session 9i)

Why this exists
---------------
Session 5's E2 test reported corr = -0.02 and R^2 ~ 0.006 between the one-month TWS change and
the ERA5 water balance, and concluded that "next month's weather does not explain the change",
which closed off the covariate avenue for four sessions.

That number is physically impossible. P - E - R *is* the change in stored water; the correlation
must be strongly positive. The likely cause is the same encoding bug fixed in session 9c: a RAW
millimetre total was correlated against a STANDARDISED anomaly, pooled across every cell on
Earth. Raw totals are dominated by the seasonal cycle and by between-cell scale (the Amazon
receives 300 mm where the Sahara receives 5), so pooling destroys the relationship even when it
is perfect within each cell.

This script measures the ceiling honestly, three ways:

  global      corr / R^2 pooled over all rows, as E2 did
  within-cell both series demeaned per cell first, which removes between-cell scale entirely
  per-horizon because the window length changes with the horizon

and then fits a ridge on the anomaly block (train rows -> validation rows, so the R^2 is honest)
to bound what a LINEAR use of these features can explain. The gradient-boosted model should beat
that bound; if it does not, the features are not being used well.

Reading it: if the anomaly encoding shows a far higher correlation than the raw encoding, E2 was
an artifact and covariate work is worth more investment (ERA5-Land, more variables, longer
windows). If both are near zero, we really are at the information ceiling and the remaining days
belong to blending and the report.

usage: python ceiling.py A          (or B)
"""
import sys
import numpy as np
import polars as pl
...
def within_cell(x, y, cell):
    """Pooled within-cell correlation: demean both series per cell, then correlate."""
...
def main():
    ...
    # raw / anomaly pairs to compare, wherever both encodings exist
    ...
    # linear ceiling on the anomaly block, fitted on training rows and scored on validation rows
