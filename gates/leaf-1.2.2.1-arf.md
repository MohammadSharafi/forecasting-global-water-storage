...
- [x] L1: all 20 prediction files exist (5 layouts x 2 arms x 2 seeds)
  CHECK: ls out/mats/pred_{Avn2,Bvn2,Cvn2,D,E}_lgb_v5x_noll_s[01]_a[01].npy 2>/dev/null | wc -l | tr -d ' ' | sed 's/^/files=/'
  EXPECT: files=20
  EVIDENCE: files=20

- [x] L2: provenance: every treatment run used 343 features and every control 340
  ...
  EVIDENCE: t343=10 | c340=10

- [x] L3: the control arm is bit-identical to the shipped d0 predictions on all 10 (layout, seed) pairs
...
- [x] L4: scored with the shared harness; log ends in a contract summary line
  CHECK: tail -1 out/goal065/arf.txt
  EXPECT: /^arf \| Avn2 \S+ Bvn2 \S+ Cvn2 \S+ D \S+ E \S+ \| wins \d\/5 \| mean \S+ \| worst \S+ \| clears-0\.003 \d\/5 \| \S+\s*$/
  EVIDENCE: arf | Avn2 -0.0009 Bvn2 +0.0026 Cvn2 +0.0027 D -0.0003 E -0.0003 | wins 3/5 | mean +0.0008 | worst +0.0027 | clears-0.003 0/5 | REJECT

- [x] L5: the summary line is in heldout.txt
