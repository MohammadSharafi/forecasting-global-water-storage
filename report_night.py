"""Assemble one readable report from everything the overnight run produced (session 9n).

out/night/ ends a full run with fifty-odd log files and four little shell fragments, and the
useful content is spread across them in the order the phases happened rather than the order a
reader needs. This gathers it into out/RUN_REPORT.md: what was decided and why first, then the
evidence, then the data and error analysis, then the compliance audit.

The point is not tidiness. The top entries are code-reviewed, and this file is most of what that
review needs -- the configuration, the decision rule, the held-out numbers behind every adopted
change, and the audit -- in one place, written as the run happened rather than reconstructed
afterwards.

usage: python report_night.py [out/night] > /dev/null   (writes out/RUN_REPORT.md)
"""
import glob
import os
import sys
...
# (log stem, heading, how many trailing lines are worth keeping; None = the whole file)
SECTIONS = [
