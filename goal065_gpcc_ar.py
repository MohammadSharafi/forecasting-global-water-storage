"""Apply the AR anchor rescaling to the GPCC model, exactly as it was validated.

goal065_joint.py measured the two changes TOGETHER by re-fitting the correction against the GPCC
model's own predictions and scoring on a held-out layout: mean -0.0085, winning 5/5 and clearing
-0.003 on all five. This applies that same fit to the FINAL file.

Note the form. In goal065_build.py the correction was a MARGINAL term added to a blend, because the
weights there had been fitted against a different model than the base. Here the base IS the model
the weights were fitted against, so the submission is the fitted combination itself:

    w4 . (gpcc model, persistence, climatology, AR)

Weights are fitted on validation labels across five layouts and never on a leaderboard score.
"""
import glob
...
def main():
    ...
    # report the held-out value once more, so the file is never built on a remembered number
    ds = []
    for L in D:
