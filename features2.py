"""v2: honest (leave-one-year-out) climatology for TRAINING rows, spatial-neighbour
anomalies, last-two-observation slope, residual target option."""
...
def assemble2(rows, cov_all, obs_all, sums, cell, loyo):
    """loyo=True: training rows -> climatology excludes the row's own contribution
    (its target for m_next, its tws_known for m_known)."""
    ...
    # previous observation before t_known (slope)
    ...
    # climatology (LOYO for training rows)
    ...
    # spatial neighbours: mean anom_known and mean SPEI_03 delta over the 8 surrounding cells (same t, t_known)
