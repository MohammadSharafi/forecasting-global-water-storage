"""Extra features (session 2):
  - NCEP-DOE Reanalysis-2 water balance (same fields as R1, prefix 'r2'), public/no login.
  - CPC leaky-bucket soil moisture (0.5 deg, mm, NOAA PSL, monthly, operational), prefix 'cpc'.
  - Wider (radius 4) neighbourhood means of dynamic features.
  - Covariate window statistics over (t_known, t] and lags of covariates relative to t.
  - Per-cell response coefficients of monthly TWS change on SPEI-1 / soil-moisture change (history only).
"""
...
def add_ext(r, tab, cols, acc_cols=()):
    """values at t and t_known, differences, and sums of acc_cols over (t_known, t]."""
...
def add_wide4(r, radius=4):
    """Neighbourhood means (excluding the centre) on the 1-degree grid via convolution, per (time, t_known) map."""
...
# The TWS-derived quantities (d1, d3, d12, trend_persist, anom_persist) also had no regional
# aggregate, and adding them was measured at -0.0008 on layout B -- not enough to keep. The
# contrast is the useful part: aggregating the FORCING pays because reanalysis P-E-R errors are
# independent between cells and average out; aggregating a TWS-derived quantity does not, because
# the TWS field is already smooth. Aggregate a noisy driver, not a smooth state.
...
def cell_response(hist):
    """per-cell OLS slopes of one-month TWS change on SPEI-1 at t and on the soil-moisture change into t (history only)."""
