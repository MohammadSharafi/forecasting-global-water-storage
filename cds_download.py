"""Download ERA5 monthly means (single levels) on a .5-centred 1-degree grid, 2001..2019.
...
The request is cut to the competition's own box plus a 5-degree margin, read from Train.csv
rather than hardcoded. The grid is 40x40 cells over tropical South America, so a global request
asks CDS for about twenty times the data that can ever be joined to a row.

ERA5_DIR overrides the destination. Use it to download into a staging directory while a run is
in flight -- arriving files change the build_ fingerprint, and a matrix built half-way through
an ERA5 download would carry a different feature set from the one built before it.
"""
