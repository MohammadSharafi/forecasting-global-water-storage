...
- step 14: ablation result. Dropping the horizon-mix reweighting (HMIX) gives D -0.0012 and E -0.0001, mean -0.0007, negative on both so it never hurts; dropping the recency ramp gives exactly +0.0000 on D; dropping both equals dropping HMIX alone. Coherent, not noise. Adopted for the final build because it is free, but recorded honestly as far too small to explain the gap to the leaders: -0.0007 validation is ~-0.0006 board against a 0.026 gap to rank 10. My "accumulated structure is costing us" hypothesis is directionally right and nowhere near the right magnitude.
- step 14: final build configuration = XF=gpcc,wgap, DROPF=bigsa,gdo, WEIGHTS=ramp, HMIX unset, rounds 410/230, smoothing 0.7/r1/it1, then the AR correction re-fitted against these models, then a mix with the record.
